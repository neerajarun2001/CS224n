...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.stack([torch.index_select(self.embeddings, 0, w[i]) for i in range(w.shape[0])]).view(batch_size, -1)
    ...
    def forward(self, w):
        ...
        h = F.relu(torch.mm(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        h = self.dropout(h)
