...
def naiveSoftmaxLossAndGradient(
    ...
    logits = outsideVectors.dot(centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    loss, gradCenterVec, gradOutsideVecs = 0, np.zeros_like(centerWordVec), np.zeros_like(outsideVectors)
    for i in indices: 
        u_i = outsideVectors[i]
        logit_i = u_i.dot(centerWordVec)
        if i == outsideWordIdx: 
            sigmoid_term = sigmoid(logit_i)-1
        else: 
            sigmoid_term = -(sigmoid(-logit_i) - 1)
            logit_i = -logit_i 
        loss += -np.log(sigmoid(logit_i))
        gradCenterVec += sigmoid_term * u_i
        gradOutsideVecs[i] += sigmoid_term * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for o_word in outsideWords:
        outsideWordIdx = word2Ind[o_word]
        l, gradCentVec, gradOutVec, = word2vecLossAndGradient(
                                        centerWordVec, 
                                        outsideWordIdx, 
                                        outsideVectors, 
                                        dataset)
        loss += l
        gradCenterVecs[centerWordIdx] += gradCentVec
        gradOutsideVectors += gradOutVec
